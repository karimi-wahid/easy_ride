...
export class RealtimeService {
  ...
  setServer(namespace: Namespace): void {   this.namespace = namespace;
  ...
  notifyDriverLocation(  rideId: string,  payload: DriverLocationEvent,): void {
    ...
    this.logger.log(    `DRIVER LOCATION SENT | rideId=${rideId} | room=${room}`,
    );
  }
  ...
  async joinDriver(
    ...
    const room = SOCKET_ROOMS.driver(driverId);
    await socket.join(room);
    this.logger.log( `DRIVER ROOM JOINED | socket=${socket.id} | driverId=${driverId} | room=${room}`, );
  }
  ...
  async joinUser(
    ...
    const room =SOCKET_ROOMS.user(userId);
    await socket.join(room);
    this.logger.log( `USER ROOM JOINED | socket=${socket.id} | userId=${userId} | room=${room}`,  );
  }
